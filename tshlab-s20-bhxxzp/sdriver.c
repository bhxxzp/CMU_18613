 ...
 */
#define PERLPROG "while(<>){ s/\\(\\d+\\)/\\(PID\\)/g; print \"$_\"}"
...
int runtrace(char *tracefile) {
    ...
    /* Filtered output files were different */
        ...
        snprintf(buf, sizeof(buf), "./colordiff.pl --verifymode -w %s %s %s\n",
...
void delete_tmpfiles(void) {
    char buf[MAXBUF];
    int ret;
    sprintf(buf, "rm -rf %s %s %s %s %s",
            test_raw_outfile, ref_raw_outfile, test_filtered_outfile,
            ref_filtered_outfile, diff_filtered_outfile);
    ret = system(buf);
    if (ret != 0) {
		error_msg(buf, ret);
        exit(1);
